...
function animate() {
  ...
  if (modelSound && modelSound.buffer && modelSource) {
    ...
    if (dist < 9 && !modelSound.isPlaying) {
      ...
      if (!infoVisible && !isXR) {
        ...
        showVRInfoText("FACT #1: Geological Nature", "Thingvellir is a National Park and UNESCO World Heritage Site. This place lies on the Mid-Atlantic Ridge. Here, you can walk between the North American and Eurasian tectonic plates, which drift apart by millimeters each year.");
      }
  ...
  if (distbook < 10 && !bookInfoShown && !isXR && controls && controls.isLocked) {
    showInfoText("FACT #4: The Icelandic Sagas at Thingvellir", "Medieval stories that preserved the memory of this place and its people. Written centuries after the events they describe, they offer glimpses into how the Alþing functioned – and how central sound and speech were to Icelandic culture. In Brennu-Njáls Saga, one phrase appears over and over – more than 14 times: Lýsi ek í heyranda hljóði að Lögbergi (I declare this in audible voice at the Law Rock.) This wasn't just a formality. It was a legal requirement. To make something official, it had to be proclaimed aloud, at Lögberg, where everyone could hear. The sagas remembered not just what was said, but where and how it was said. Sound mattered. Being heard mattered.", "images/book.jpeg");
    bookInfoShown = true;
    console.log('✅ Book info shown');
  } else if (distbook < 10 && !bookInfoShown && isXR && !vrInfoVisible) {
    showVRInfoText("FACT #4: Icelandic Sagas", "Medieval stories preserved memory of this place. In Brennu-Njals Saga, one phrase appears over 14 times: I declare this in audible voice at the Law Rock. This was a legal requirement. It had to be proclaimed aloud, at Lögberg, where everyone could hear.");
    bookInfoShown = true;
  ...
  if (balloonSource && balloonSound && balloonSound.buffer) {
    ...
    if (dist < 10 && !balloonInfoShown && !isXR) {
      ...
      showVRInfoText("FACT #2: Distinctive Echo", "Due to its distinctive geographical landscape, Thingvellir has a highly recognizable acoustic footprint. You can always perceive a characteristic echo caused by the geological formations.");
      balloonInfoShown = true;
    } else if (dist >= 10) {
  ...
  if (speakerSource) {
    ...
    if (distspeaker < 7 && !speakerInfoShown && !isXR && controls && controls.isLocked) {
      ...
      showVRInfoText("FACT #3: Orality at Lögberg", "At the Lögberg (Law Rock), Icelanders gathered to proclaim laws and deliver judgments. The Law Speaker recited aloud the laws of the Commonwealth. The low ambient noise made Lögberg ideal for speech intelligibility.");
      speakerInfoShown = true;
